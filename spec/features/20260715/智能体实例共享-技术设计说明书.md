...
| 2026-07-20 | V1.3.0 | CR-002 增加运行终态、事件发送、Web 增强和 Plan 执行代码质量治理设计，不改变公共调用契约 |

## 13. CR-002 代码质量与缺陷治理覆盖设计

### 13.1 设计边界

- 保持 `Agent.start/stream/call`、`AgentResumeRequest`、Redisson 任务协调方式和现有流式 JSON 外层协议不变。
- 不新增 AgentFactory、协调器 SPI、审批平台、配置项、持久化结构或下游接入协议。
- 只提取当前实现中的重复代码、拆分过长职责、删除确认无引用的代码，并修复已能由源码路径和确定性测试证明的失败分支。

### 13.2 BaseAgent 终态与事件发送

- `finishRun` 与 `pauseForNativeApproval` 统一复用安全清理方法。ChatMemory、供应商资源、TaskManager 和 Hook 任一扩展失败时记录技术标识和 cause，但不得阻止事件流与 `completion()` 收口。
- Run 的主订阅和伴随任务在真实终态统一释放；资源关闭与用户取消使用不同入口，避免终态清理再次触发取消语义。
- 每次事件发送必须检查 Reactor `EmitResult`。同一 Run 内的发送串行化，避免并发工具或 Plan 任务产生 `FAIL_NON_SERIALIZED` 后静默丢事件。
- 计时从 Run 真正开始执行时计算；首响应与总响应字段统一表达毫秒耗时，不再混用 epoch 时间戳和 duration。
- 动态参数使用固定标签和转义后的 name/value 表达，不允许调用方 key 直接成为 XML 标签。

### 13.3 ReAct 与 Web Search

- Web Builder 继承现有 `BaseAgentBuilder`，只保留工具注册表和 ReAct 专属配置。
- Web 参数/结果解析属于进度和引用增强。非法 JSON、缺少解析器、解析结果为 null 或解析异常不得把已成功的工具调用改成失败，只记录安全告警并跳过对应增强。
- 搜索与提取结果继续保存在每 Run 上下文；使用现有 `WebToolResult` 统一去重和序列化，不改变 `REFERENCE` 事件类型。

### 13.4 Plan-Execute

- 将 Graph 构建、任务执行、审批节点适配和引用/结果收集从 `PlanExecuteAgent` 拆为包内组件；Agent 只负责运行编排。
- 每个 PlanTask 的内部执行器只注入计划指定的 `toolName` 对应工具，禁止模型调用同一 Agent 上其他工具。
- `maxToolRetries` 默认值和公共 Builder 行为保持不变；本 CR 不改变已有重试契约，但在注释中明确副作用工具需要由接入方保证幂等。
- 异步任务必须保留能够中断真实执行线程的 Future/Disposable，并纳入当前 Run 的资源树。超时或取消后禁止迟到结果写回、事件输出和后继波次调度。
- Builder 只创建最终使用的线程池，不再先创建默认池再立即 shutdown/rebuild。

### 13.5 验证与停止条件

- 先增加终态清理异常、事件并发、Web 解析、Plan 工具隔离和超时/取消迟到结果的失败测试，再实施最小修复。
- 删除代码前执行生产与测试引用扫描；只删除仓库内无调用且不属于当前公共契约的类型、字段和配置。
- 若实现需要改变公共 API、Redis 协议、审批恢复输入或流式消息结构，停止 CR-002 并返回变更规划，不在本次实现中扩大范围。
