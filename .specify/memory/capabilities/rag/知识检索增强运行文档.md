...
| `LangChain4jDocumentParserFacade` | LangChain4j 统一解析入口 | 启动期至运行期 | 已验证 |
...
```mermaid
flowchart TD
    A["Spring Boot 加载 RAG 自动配置"] --> B["绑定 sys.rag.document-parser"]
    ...
    E2 --> F2["装配 LangChain4jDocumentParserFacade"]
```

两个框架的注册表因 payload 类型不同而独立；共享 MinerU 协议组件使用缺省 Bean 条件，双框架同时装配时不重复创建协议实现。
