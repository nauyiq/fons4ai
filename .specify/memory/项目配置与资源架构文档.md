...
> 文档层级：项目级
> 文档状态：基线已建立，RAG 文档解析配置与资源已汇总，Agent LangChain4j 配置已补充
> 更新日期：2026-07-31

## 1. 配置与资源架构定位

- 配置架构目标：通过 Spring Boot 配置属性和条件自动配置，按需装配 Agent、工具、RAG 与 AI 服务能力。
- 核心配置主线：`sys.tavily`、`sys.image-generation`、`sys.rag.vector`、`sys.multiple-modal`、`sys.rag.document-parser`、`sys.agent.langchain`，以及 Spring AI、LangChain4j 与外部基础设施连接配置。
...
| `sys.rag.document-parser` | RAG | 默认解析 provider 及 MinerU 开关、地址、后端、超时和文件上限 | 两个框架配置属性与公共 `MinerUClientOptions` | DEFAULT 固定 native；MinerU 默认关闭；10s/5m/100MB | 外部文档传输、超时、容量和版本协同 |
...
| `sys.agent.langchain` | Agent 编排 | LangChain4j Agent 对话记忆窗口、工具调用轮次上限和记忆存储类型 | `LangChain4jAgentProperties` | maxMemoryMessages=10；maxSequentialToolsInvocations=10；memory.type 默认 in-memory；redis.keyPrefix=`fons4ai-agent:memory:`；redis.ttlHours=24 | memory.type=redis 时 RedissonClient 不可用会降级为 in-memory |
...
| LangChain4j ChatMemoryStore 键 | Agent 编排 | 运行期，按 keyPrefix 隔离，TTL 默认 24 小时 | 上层应用基础设施 | `RedissonChatMemoryStore`（memory.type=redis 时） | Redis 不可用时降级为 InMemoryChatMemoryStore；需明确 TTL 与会话生命周期 |
...
| MinerU HTTP 服务与文档源 | RAG | 显式 MinerU 解析期间 | 上层应用配置的外部服务；文档源由调用方管理 | 共享 MinerU client/provider、框架 adapter | 默认关闭；限制超时和大小；关闭临时资源；人工安全 Gate |
...
| MinerU 地址、启用开关、超时与文件上限 | 应用配置与文档解析属性类 | 上层应用运维/安全/配置所有者 | RAG 文档解析自动配置 | 启用后可能向外部服务发送敏感文档；错误配置导致不可用或容量风险 |
| 多模态与图像生成服务参数 | 应用配置与属性类 | 上层应用配置所有者 | AI 能力实现 | 服务商变更可能影响请求/响应兼容性 |
| Redis/Redisson 连接 | 上层应用基础设施配置 | 上层应用基础设施所有者 | `AgentTaskManager`、`RedissonChatMemoryStore` | 影响任务互斥、停止和中断；影响 LangChain4j Agent 记忆持久化 |
...
| 文档解析显式外发 | MinerU 等外部解析 provider | 默认 native 不外发；只有显式选用且 capability 校验通过才可调用外部 provider；不记录正文和认证信息 | 代码与测试已验证，生产人工 Gate 待确认 |
...
| CQ-004 | MinerU 真实部署地址、网络边界、文档传输合规性、容量和告警规范 | RAG 文档解析生产接入 | 完成人工安全 Gate 与真实环境验证后补充 |
