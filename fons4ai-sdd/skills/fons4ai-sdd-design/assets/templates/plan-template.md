# <功能名称> 技术方案

> 功能标识：`<feature-slug>`
> SDD 等级：`S1|S2`
> 来源需求：`specs/features/<feature-slug>/spec.md`
> 文档状态：初稿
> S1 填写规则：保留必备章节；无状态流转、数据结构变更、API 契约、迁移回滚或图表时，写 `不适用，原因`，不得编造未验证内容。

## 仓库事实

- 现有模块：
- 现有模式：
- 可复用工具或组件：
- 测试约定：
- 已调研的代表性文件：

## 知识库事实

- 业务架构：
- 技术架构：
- 数据架构：
- SQL DDL 文件：
- 治理约束：
- 其他真理源：
- 真理源事实与代码事实冲突：

## 设计摘要

描述满足 AC 与 REQ 范围的最简可行实现方案。

## 关键业务规则与策略设计

### 业务规则落地

| 业务规则 | 来源 | 技术落点 | 领域/应用对象 | 数据依赖 | 验证方式 |
| --- | --- | --- | --- | --- | --- |
| <规则名称> | REQ-001/AC-001/仓库事实 | <模块/类/方法/配置> | <领域对象/应用服务/策略类> | <表/字段/外部数据> | <测试或手动检查> |

### 策略决策流程

```mermaid
flowchart TD
    A[输入业务上下文] --> B{策略条件判断}
    B -->|命中策略 A| C[执行策略 A]
    B -->|命中策略 B| D[执行策略 B]
    B -->|不满足条件| E[默认策略或异常处理]
    C --> F[输出业务结果]
    D --> F
    E --> F
```

- 图示状态：不适用，原因 | 已根据仓库事实补全 | 部分节点待确认
- 策略扩展点：
- 默认策略或兜底策略：
- 异常与拒绝策略：

### 核心业务方案落地

| 业务能力 | 落地模块 | 入口/API/消息 | 应用编排 | 领域行为 | 持久化/查询 | 测试覆盖 |
| --- | --- | --- | --- | --- | --- | --- |
| <能力名称> | <模块> | <入口> | <应用服务/用例> | <领域对象/方法> | <Repository/Mapper/SQL> | <单元/集成/契约/手动> |

## 架构设计

- 目标模块或层级：
- 依赖方向：
- 领域层/应用层/基础设施边界：
- 复用的扩展点：
- 新增或调整的抽象：无 | <名称与原因>

## 实现方案

- 主实现路径：
- 复用的既有代码或模式：
- 新增或调整的文件：
- 向后兼容策略：
- 会影响任务拆解的实现说明：

## 领域建模决策

- 是否适用 DDD-lite：是 | 否，原因
- 核心领域对象：
- 业务规则与不变量：
- 状态流转：
- 值对象候选：
- 领域服务候选：
- 应用层编排：
- 贫血模型例外：无 | 简单 CRUD | 只读查询 | DTO/投影 | 兼容原因 | 领域抽象尚不稳定，原因
- 基础设施依赖边界：

## 关键规则代码片段

使用短伪代码或类代码片段说明关键规则。这里不是最终生产代码。

```text
不适用，原因
```

- 规则来源：REQ-001 | AC-001 | 仓库事实
- 计划复用的既有类型或工具：
- 已覆盖的边界情况：

## 状态流转设计

| 状态 | 触发事件 | 前置条件 | 目标状态 | 失败处理 | 幂等要求 |
| --- | --- | --- | --- | --- | --- |
| 不适用，原因 |  |  |  |  |  |

图示：不适用，原因 | 有事实依据时使用 Mermaid stateDiagram

## 数据流

- 请求或输入：
- 校验：
- 领域行为：
- 持久化：
- 外部协作：
- 响应或输出：

## 工具包与依赖决策

- 可复用项目工具或组件：
- 已引入三方工具：无 | Hutool | Apache Commons | Guava | 其他
- 字符串/集合/日期时间/IO/Bean/判空/断言的工具选择：
- 是否需要新增依赖：否 | 是，原因
- 备选方案：
- 新增依赖确认状态：不适用 | 用户或设计已确认 | 暂缓
- 可读性与复杂度说明：

## 影响范围

| 影响区域 | 文件或模块 | 改动类型 | 原因 |
| --- | --- | --- | --- |
|  |  | 新增/更新/删除 |  |

## API 与契约细节

- API/UI/契约变化：无 | <详情>
- 请求结构：
- 响应结构：
- 兼容性影响：
- 契约文件：无 | `contracts/<name>.md`

## 数据结构变更

| 对象/表/DTO | 字段 | 类型 | 变更 | 默认值/约束 | 兼容策略 |
| --- | --- | --- | --- | --- | --- |
| 不适用，原因 |  |  |  |  |  |

- 数据库/服务：
- 业务模型：
- 是否需要迁移：是/否
- 迁移脚本证据：有，`<path>` | 无 | 待确认
- 是否需要 DDL 同步：否 | 是
- DDL 文件动作：无 | 新增 | 更新 | 重命名
- DDL 文件影响：无 | `.specify/sql/<database_or_service>/<business_model>.sql` | `.specify/sql/pending/<business_model>.sql`
- 无迁移脚本处理：不适用 | 基于实体/ORM/Mapper/查询 SQL/用户事实生成 SQL 知识文件，并标记推断或待确认字段
- DDL 同步时机：同一实现任务 | 独立任务 | 暂缓，说明负责人和原因

## 错误与异常处理

- 业务错误：
- 校验错误：
- 外部依赖失败：
- 重试或补偿：
- 日志与敏感数据脱敏：

## 事务与一致性

- 事务边界：
- 一致性模型：
- 并发或锁：
- 幂等：
- 缓存/MQ/定时任务一致性：

## 迁移与回滚细节

- 迁移步骤：不适用，原因 | <步骤>
- 回滚步骤：
- 兼容窗口：
- 数据修复或回填：
- 运维说明：

## 关键决策

| 决策 | 理由 | 备选方案 |
| --- | --- | --- |
|  |  |  |

## AC 映射

| AC | 关联需求 | 设计覆盖 |
| --- | --- | --- |
| AC-001 | REQ-001 |  |

## 风险与回滚

- 风险：
- 缓解措施：
- 回滚：

## 验证策略

- 单元测试：
- 集成测试：
- 契约/API 测试：
- 持久化或迁移检查：
- 手动检查：

## 知识同步影响

- 是否需要更新业务架构：否
- 是否需要更新技术架构：否
- 是否需要更新数据架构：否
- 是否需要更新其他真理源：否
- 是否需要更新 SQL DDL：否
- SQL DDL 文件：
- DDL 分组规则：仅同一数据库/服务且业务模型强耦合时合并；不同数据库/服务必须拆分
- 知识同步标记：Knowledge Sync Needed: no
- 说明：

## 待确认问题

- 问题：
