...
- `spec.md` 是面向业务的需求说明书，必须包含背景与目标、业务范围、角色与业务场景、业务流程、业务规则、功能需求、业务数据说明、业务影响和可测试验收标准；需求点使用 `REQ-001`、`REQ-002` 等编号，验收标准使用 `AC-001`、`AC-002` 等编号，并建立 REQ 到 AC 的映射。
...
- 只有用户明确要求“先按假设生成草案”时，才允许生成带 `文档状态：草案-待确认` 的草案文档；草案不得进入设计、任务或实现阶段。
- 需求和变更澄清属于 agent 内部前置流程。正式 `spec.md` 和正式 CR 不展示澄清门禁表格、澄清状态或逐轮澄清记录。
- `spec.md` 应使用业务术语描述业务对象、业务数据、流程、规则和验收结果；模块、类、表、字段、DDL 路径、MCP 信息和技术架构细节统一放入 `plan.md` 或后续技术产物。
- `plan.md` 是技术设计说明书，必须覆盖每条 AC，并记录总体架构、关键业务规则与策略落地、核心业务场景实现、数据流、领域建模、关键规则代码片段或伪代码、状态流转、接口与契约、数据模型与 ER 设计、事务一致性、迁移回滚、风险和验证策略；调研仓库与知识库属于内部过程，不在正式方案中展示事实清单。
- `plan.md` 图表按事实和复杂度生成：核心调用链按需使用时序图，复杂策略按需使用流程图，状态变化按需使用状态图；新增表、关系变化或多表协作时补充 ER 图，单字段或单索引调整可写 `不适用，原因`。
