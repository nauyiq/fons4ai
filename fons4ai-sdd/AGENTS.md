...
- `需求说明书.md` 面向业务，使用业务术语描述背景、范围、角色场景、需求、规则、流程、数据口径、影响和验收标准；技术细节放入 `<功能中文名>-技术设计说明书.md`。
- `技术设计说明书.md` 面向实现，描述总体架构、核心规则落地、数据流、领域建模、关键代码片段、状态流转、接口契约、数据模型、事务一致性、迁移回滚和验证策略。
- `<功能中文名>-任务规划.md` 面向执行，每个任务必须包含 `AC:`、`Files:`、`Verification:`、`Quality:`、`Done:`。
...
- 生成 `<功能中文名>-任务规划.md` 或 CR 增量任务后必须暂停。用户回复 `执行`、`开始实现`、`继续执行` 时默认执行全部未完成任务；用户回复 `执行 T001,T002` 时只执行指定任务。
