package com.fons.cloud.ai.rag.document.splitter;
 ...
 * 支持保留元数据、父子分段关系等高级特性
 * @author andyflury （<a href="https://github.com/langchain4j/langchain4j/issues/574">...</a> ）
 */
public class MarkdownHeaderTextSplitter extends TextSplitter {
    ...
    private List<DocumentWithMetadata> aggregateLinesToChunks(List<Line> lines) {
        ...
        for (Line line : lines) {
            ...
            if (!aggregatedChunks.isEmpty() && aggregatedChunks.getLast().getMetadata().equals(line.getMetadata())) {
                Line last = aggregatedChunks.getLast();
                last.setContent(last.getContent() + "  \n" + line.getContent());
            }
            // 情况2：元数据不同但上一行以标题结尾且未剥离标题，则也合并
            // 这样可以将标题和其下的第一段内容合并在一起
            else if (!aggregatedChunks.isEmpty() && !aggregatedChunks.getLast().getMetadata().equals(line.getMetadata())
                    && aggregatedChunks.getLast().getMetadata().size() < line.getMetadata().size()
                    && aggregatedChunks.getLast().getContent().split("\n")[aggregatedChunks.getLast().getContent().split("\n").length - 1].startsWith("#") && !stripHeaders) {
    ...
    private record Header(int level, String name, String data) {
